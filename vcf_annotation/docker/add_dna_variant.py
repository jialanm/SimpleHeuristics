...
def add_annotation(mt, annotation_ht, splice_ai_ht):
    key_mt = annotation_ht[mt.row_key]
    key_mt.describe()

    key_mt_splice_ai = splice_ai_ht[mt.row_key]
    print("test1")
    splice_ai_ht.describe()
    print("test2")
    key_mt_splice_ai.describe()
    if "cohort_AC" in mt.info:
        mt = mt.annotate_rows(info=hl.struct(
            ...
            splice_ai=convert_str_to_float(splice_ai_ht[mt.row_key].spliceai_ds_max),
        ))
    else:
        mt = mt.annotate_rows(info=hl.struct(
...
def main():
    ...
    splice_ai_ht = hl.read_table(gnomad_splice_ai)
    print("splice_ai_ht")
    splice_ai_ht.show()
